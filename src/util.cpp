...
#else
    const char* pszModule = "bitcrystal";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitCrystal
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitCrystal
    // Mac: ~/Library/Application Support/BitCrystal
    // Unix: ~/.bitcrystal
#ifdef WIN32
    // Windows
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bitcrystal.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No bitcrystal.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override bitcrystal.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitcrystal.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BitCrystal will not work properly.");
