												...
												bool *imageFromSharedCache)
{
	...
	err = mach_vm_read_overwrite(task, remote_header, size, (vm_address_t)&header, &size);
	RDFailOnError("mach_vm_read_overwrite()", fail);
	...
	*imageFromSharedCache = ((header.flags & kImageFromSharedCacheFlag) == kImageFromSharedCacheFlag);
	...
	if (header.magic != MH_MAGIC && header.magic != MH_MAGIC_64) {
		syslog(LOG_NOTICE, "liblorgnette ERROR: found image with unsupported architecture at %p, skipping it.\n", remote_header);
		return 0;
	}
	...
	for (uint32_t i = 0; i < header.ncmds; i++) {
		err = mach_vm_read_overwrite(task, command_addr, size, (vm_address_t)&command, &size);
		RDFailOnError("mach_vm_read_overwrite()", fail);
	...
	err = mach_vm_read_overwrite(task, symtab_addr, size, (vm_address_t)&symtab, &size);
	RDFailOnError("mach_vm_read_overwrite", fail);
	...
	if (sixtyfourbit) {
		...
		err = mach_vm_read_overwrite(task, text_addr, size, (vm_address_t)&text, &size);
		RDFailOnError("mach_vm_read_overwrite", fail);
		...
		for (uint32_t i = 0; i < symtab.nsyms; i++) {
			...
			err = mach_vm_read_overwrite(task, sym_addr, size, (vm_address_t)&sym, &size);
			RDFailOnError("mach_vm_read_overwrite", fail);
			sym_addr += size;
