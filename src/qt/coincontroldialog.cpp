...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        // Fee
        int64 nFee = IS_FREE != 1 ? nTransactionFee * (1 + (int64)nBytes / 1000) : 0;
        
        // Min Fee
        int64 nMinFee = IS_FREE != 1 ? CTransaction::nMinTxFee * (1 + (int64)nBytes / 1000) + CTransaction::nMinTxFee * nQuantityDust : 0;
       if (IS_FREE != 1 ? CTransaction::AllowFree(dPriority) && nBytes < 5000 : true)
            nMinFee = 0;
        
        nPayFee = IS_FREE != 1 ? max(nFee, nMinFee) : 0;
