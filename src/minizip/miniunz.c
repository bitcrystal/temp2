  ...
  while (1)
    {
      ...
      hold = *p;
      *p = 0;
      if ((mymkdir__miniunz(buffer) == -1) && (errno == ENOENT))
        {
...
int do_extract_currentfile__miniunz(unzFile uf,const int* popt_extract_without_path,int* popt_overwrite,const char* password)
{
    ...
    if ((*filename_withoutpath)=='\0')
    {
        if ((*popt_extract_without_path)==0)
        {
            ...
            mymkdir__miniunz(filename_inzip);
        }
    }
    else
    {
        ...
        if ((skip==0) && (err==UNZ_OK))
        {
                                ...
                                (filename_withoutpath!=(char*)filename_inzip))
            {
                ...
                *(filename_withoutpath-1)='\0';
                makedir__miniunz(write_filename);
        ...
        if (fout!=NULL)
        {
            ...
            if (err==0)
                change_file_date__miniunz(write_filename,file_info.dosDate,
                                 file_info.tmu_date);
        }
...
int do_extract__miniunz(unzFile uf,int opt_extract_without_path,int opt_overwrite,const char* password)
{
    ...
    for (i=0;i<gi.number_entry;i++)
    {
        if (do_extract_currentfile__miniunz(uf,&opt_extract_without_path,
                                      &opt_overwrite,
                                      password) != UNZ_OK)
            break;
...
#            endif
    ...
    else if (opt_do_extract==1)
    {
        ...
        if (filename_to_extract == NULL)
            return do_extract__miniunz(uf,opt_do_extract_withoutpath,opt_overwrite,password);
        else
            return do_extract_onefile__miniunz(uf,filename_to_extract,
